...
#		for my $bampath (@modbams){
			...
			if (exists($bam{$bampath}{"$readid"})){
				...
				for my $mod (@modlist){ 
					...
					if ($CGn > 0 && $mm) {
						...
						if (scalar(keys(%CGhash)) > 0){

							#print "Too many CG:", scalar(keys(%CGhash)), "positioned as follows ", join(",", keys(%CGhash)), " at $bline \n";

							$extra_CG_pos = join(",", keys(%CGhash));
							$extra_CG_count = scalar(keys(%CGhash));
						}

					} elsif ($CGn > 0 && !($mm)){
						$printmod = join("\t",@splitb[(0,2,3)]) . "\t$CGn\tMM_Undetected\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\n";
					}
					...
					$printmod = join("\t",@splitb[(0,2,3)]) . "\t$CGn\t$Nmod\t$Cn\t$mod_pos_Cmod\t$mod_pos_allCG\t$modvalues_CG\t$CGn_within\t$Nmod_within\t$mod_pos_Cmod_within\t$mod_pos_allCG_within\t$modvalues_CG_within\t$flag\t$leftclip\t$len\t$rightclip\t$extra_CG_pos\t$extra_CG_count\n";
		...
		if ($check_found==0){
			for my $mod (@modlist){
				...
				open(OUT, ">>$modoutdir/$cropname.$outmod{$mod}");
				print OUT "$readid\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\n";
				close(OUT);
			}
		}
