...
option_list = list(
  make_option(c("-o", "--output_dir") ,        action="store",        default=NA,      type='character', help=""),
  ...
  make_option(c("-i", "--stats_path"),         action="store",        default=NA,      type='character', help=""),
  make_option(c("-s", "--samplesheet"),        action="store",        default=NA,      type='character', help="path to tab separated file containing <sample_id_without_extension>\t<BXX>\t<path_to_barcoding_summary.txt> where XX is the two digits number of the barcode" ),
  
  
  make_option(c("-p", '--pattern'           ), action="store",        default=NA,      type="character"),
  make_option(c("-F", '--fastq_path'        ), action="store",        default=NA,      type="character"),
  make_option(c("-M", '--merged_fastq_path' ), action="store",        default=NA,      type="character"),
  make_option(c("-B", '--barcoding_path'    ), action="store",        default=NA,      type="character", help= "path in which basecall results are stored"),
  make_option(c("-S", '--barcoding_suffix'    ), action="store",      default="GUPPY_DEM_EITHER/barcoding_summary.txt", type="character", help="path from barcoding_path to barcoding_summary.txt file [default: GUPPY_DEM_EITHER/barcoding_summary.txt]"),
  
  make_option(c("-C", '--force_config'    ),   action="store_true",   default=F,       type="character", help="forces the use of fastq_path, barcoding_path and merged_fastq_path from config file (only if samplesheet is not provided)" )
