...
pore <- pored
laby <- unique(pore$Channel.State)
...
ciao <- as.numeric(pore$Channel.State)
...
p <- ggplot(pore, aes(fill=Channel.State, y=State.Time..samples., x=Experiment.Time..minutes.)) + geom_bar(position="fill", stat="identity")
print(p)
pdf(out)  
print(p)
dev.off()
