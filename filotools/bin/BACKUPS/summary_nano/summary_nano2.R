...
pore$newstate <- mapvalues(pore$Channel.State, orig_labels, target_labels)
pore$statecol <- mapvalues(pore$Channel.State, orig_labels, lab_colors)


p <- ggplot(pore, aes(fill=newstate, y=State.Time..samples., x=Experiment.Time..minutes.)) + 
#p <- ggplot(pore, aes(fill=pore$statecol, y=State.Time..samples., x=Experiment.Time..minutes.)) + 
