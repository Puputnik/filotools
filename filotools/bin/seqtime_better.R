...
option_list = list(
 ...
 # make_option(c("-f", "--filter_tresholds"),   action="store",        default="60:60", type="character", help="front and rear threshold for barcode detection, fomatted as <front_thr>:<rear_thr> [default: 60:60]"),
  ...
  make_option(c("-B", '--basecall_path'    ), action="store",        default=NA,      type="character", help= "path in which barcoded results are stored. this path should include one subfolder for each run with filo's naming layout LXX_FATXXXXX_countX. (used only if samplesheet is not provided)"),
  make_option(c("-S", '--basecalling_suffix'    ), action="store",      default="sequencing_summary.txt", type="character", help="path from basecall_path subfolders to sequencing_summary.txt file [default: none]. (used only if samplesheet is not provided)"),
  
  make_option(c("-C", '--force_config'    ),   action="store_true",   default=F,       type="character", help="forces the use of fastq_path/basecall_path/merged_fastq_path (only if samplesheet is not provided) and stats_path (always) from config file " )
  #make_option(c("-h", "--help"), action="store_true", default=FALSE, help="Show feets")
  
)

usage_string <- "creates .time files with read ids (first column),info about sequencing time (second column), and library info (third column).

if using samplesheet

filotools seqtime -i <stats_path> -s <samplesheet_path> -o <output_dir>
...
filotools seqtime -p '*.stats' -F <fastq_path> -M <merged_fastq_path> -B <basecall_path> -o <output_dir>
filotools seqtime -p '*.stats' -C -o <output_dir>
...
#opt$samplesheet <- "/tank/USB3/LB_ANALYSIS_2022/BAM_FINAL/STATS/samples.tsv"
#opt$stats_path <- "/tank/USB3/LB_ANALYSIS_2022/BAM_FINAL/STATS"
#opt$pattern <- "*.stats"
...
if (!is.na(opt$samplesheet)){
  ...
  #set_ignored(c("pattern","fastq_path","basecall_path","merged_fastq_path","barcoding_suffix"))
  ...
  print("confl")
  print(confl)
  if (opt$force_config) {opt <- inherit_args(opt, confl,sets=c("fastq_path","basecall_path"), checknull = T)}
  print("opt")
  print(opt)
  check_def(c("pattern","fastq_path","basecall_path"))
  sname_tot <- c()
  ...
  for (i in basenames){
    ...
    if (any(grepl(i,list.files(opt$fastq_path)))){
      ...
      names(temp_list) <- file.path(opt$basecall_path, paste(field(i,5),"_",field(i,6),"_count",field(i,7),sep=""), opt$basecalling_suffix)
      ...
      if (any(grepl(i,list.files(opt$merged_fastq_path)))){
        ...
        for (s in sub_files){
          ...
          names(temp_list) <- file.path(opt$basecall_path, paste(field(s,5),"_",field(s,6),"_count",field(s,7),sep=""), opt$basecalling_suffix)
          sample_list[[i]] <- append(sample_list[[i]], temp_list)
          libraries <- c(libraries,names(temp_list))
        }
      }
...
for (l in unique(libraries)){
  ...
  temp_summ <- data.frame(temp_summ[,c("read_id",	"start_time")])
  #rownames(temp_summ) <- temp_summ$read_id
  temp_summ$lib <- l
  ...
  #  lib_list <- temp_summ
  #} else {
  #  lib_list <- rbind(lib_list, temp_summ)
  #}
  temp_summ <- NULL
}

if (is.na(opt$output_dir)){opt$output_dir <- file.path(opt$stats_path,"SEQ_TIME")}
...
for (i in basenames){
  ...
  me <-  match(stats$X4, summa$read_id)

  write.table(summa[me,], file=file.path(opt$output_dir, paste(i,".time",sep="")), quote=F, row.names = F, col.names = F)
