...
if (!is.na(opt$samplesheet)){
  ...
  for (i in basenames){
    ...
    if (any(grepl(i,list.files(opt$fastq_path)))){
      ...
      names(temp_list) <- file.path(opt$basecall_path, paste(field(i,5),"_",field(i,6),"_count",field(i,7),sep=""), opt$barcoding_suffix)
...
for (i in basenames){
  ...
  me <-  match(stats$X4, lib_list$read_id)

  write.table(lib_list[me,], file=file.path(opt$output_dir, paste(i,".time",sep="")), quote=F, row.names = F, col.names = F)
