...
if ($auto == 1){
	...
	OUTER: foreach my $q (@stats){
		...
		if ( -e "$config{fastq_path}/$cropped.fastq" || -e "$config{fastq_path}/$cropped.fastq.gz" ){
			...
			push(@modbams, "$config{basecall_path}/$libraryname/mod_mappings.bam")
		} else {
...
#		for my $bampath (@modbams){
			...
			if (exists($bam{$bampath}{"$readid"})){
				...
				for my $mod (@modlist){ 
					...
					if ($CGn > 0 && $mm) {
						...
						$printmod = join("\t",@splitb[(0,2,3)]) . "\t$CGn\tMM_Undetected\tNA\tNA\tNA\tNA\n";
					}
					...
					$printmod = join("\t",@splitb[(0,2,3)]) . "\t$CGn\t$Nmod\t$Cn\t$mod_pos_Cmod\t$mod_pos_allCG\t$modvalues_CG\n";
		...
		if ($check_found==0){
			for my $mod (@modlist){
				...
				open(OUT, ">>$modoutdir/$cropname.$outmod{$mod}");
				print OUT "$readid\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\n";
				close(OUT);
			}
		}
